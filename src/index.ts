...
function handleMiscMessage(data: any) {
  ...
  if(match) {
    ...
    if(numUnread > 0) {
      if(settings.chattabsToggle) {
        ...
        const bodyText = `You have ${numUnread} unread message${numUnread > 1 ? 's' : ''}.<br><br>Tip: To send a reply message, prefix it with "m;"`;
  ...
  match = msg.match(/^The following message was sent to \w+:/m);
