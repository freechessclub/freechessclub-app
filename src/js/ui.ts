...
function initShortcuts() {
  ...
  $('#custom-shortcuts-button').on('click', () => {
    ...
    $('#shortcut-input-group').width($('#shortcut-input-group').width());
  });
  ...
  $('#shortcut-key').on('input', (e) => {
    const shortcutKey = ($('#shortcut-key').val() as string).trim();
    if(!shortcutKey) {
      const menuItem = $('#shortcuts-list .active');
      if(menuItem.length) {
  ...
  $('#shortcut-key').on('keydown', (e) => {
    const isPrintable = e.key.trim().length === 1;
    if(!isPrintable)
      return;
    ...
    const modKey = isMac() ? 'Meta' : 'Ctrl';
    const name = `${modKey}+Shift+${e.key.toUpperCase()}`; 
