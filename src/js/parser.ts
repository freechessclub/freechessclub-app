...
import { parseDate, splitBeforeAfterMatch } from './utils';
...
export class Parser {
  ...
  private _parse(msg: string) {
    ...
    if(match != null && match.length >= 34) {
      const parts = splitBeforeAfterMatch(msg, match); // Split the style 12 line from the rest of the message
    ...
    if (match != null && match.length > 2) {
      const parts = splitBeforeAfterMatch(msg, match); // split the matching line from any additional text
      if(parts.before || parts.after) 
        return [this._parse(parts.before), this._parse(parts.matching), this._parse(parts.after)].flat();
