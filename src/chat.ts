...
export class Chat {
  ...
  constructor() {
    ...
    $(document).on('shown.bs.tab', '#tabs button[data-bs-toggle="tab"]', (e) => {
      ...
      if(!tabData.scroller) {
        tabData.scroller = this.createVirtualScroller(contentPane, tabData.messages);
  ...
  public createTab(name: string, showTab = false) {
    ...
    if(!this.tabData.hasOwnProperty(from)) {
      ...
      if(tabContent.hasClass('active')) {
        this.tabData[from].scroller = this.createVirtualScroller(tabContent, messages);
   ...
   */
  createVirtualScroller(tabContent, messages) {
